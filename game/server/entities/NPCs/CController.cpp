...
void CController::Spawn()
{
	...
	SetFieldOfView(VIEW_FIELD_FULL);
...
void CController::StartTask(const Task_t& task)
{
	...
	case TASK_GET_PATH_TO_ENEMY_LKP:
		{
			if (BuildNearestRoute(m_vecEnemyLKP, GetViewOffset(), task.flData,
			                      (m_vecEnemyLKP - GetAbsOrigin()).Length() + 1024))
			{
				TaskComplete();
			}
			else
	...
	case TASK_GET_PATH_TO_ENEMY:
		{
			...
			if (BuildNearestRoute(pEnemy->GetAbsOrigin(), pEnemy->GetViewOffset(), task.flData,
			                      (pEnemy->GetAbsOrigin() - GetAbsOrigin()).Length() + 1024))
			{
				TaskComplete();
			}
			else
	...
	if (m_flShootEnd > gpGlobals->time)
	{
		...
		while (m_flShootTime < m_flShootEnd && m_flShootTime < gpGlobals->time)
		{
			...
			if (m_hEnemy != NULL)
			{
				...
				vecDir = Intersect(vecSrc, m_hEnemy->BodyTarget(GetAbsOrigin()), m_vecEstVelocity,
				                   gSkillData.GetControllerSpeedBall());
				float delta = 0.03490; // +-2 degree
				vecDir = vecDir + Vector(RANDOM_FLOAT(-delta, delta), RANDOM_FLOAT(-delta, delta),
				                         RANDOM_FLOAT(-delta, delta)) * gSkillData.GetControllerSpeedBall();
...
//	if ( CVAR_GET_FLOAT("stopmove" ) != 0 )
	...
	do
	{
		...
		flDist = 0;
		if (CheckLocalMove(GetAbsOrigin(), GetAbsOrigin() + vecDir * flCheckDist, pTargetEnt, &flDist) !=
			LOCALMOVE_VALID)
		{
...
int CController::CheckLocalMove(const Vector& vecStart, const Vector& vecEnd, const CBaseEntity* const pTarget,
                                float* pflDist)
{
	TraceResult tr;

	UTIL_TraceHull(vecStart + Vector(0, 0, 32), vecEnd + Vector(0, 0, 32), dont_ignore_monsters, Hull::LARGE, edict(),
	               &tr);
