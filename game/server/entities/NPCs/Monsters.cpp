...
void CBaseMonster::Look(int iDistance)
{
	...
	if (!GetSpawnFlags().Any(SF_MONSTER_PRISONER))
	{
		...
		int count = UTIL_EntitiesInBox(pList, 100, GetAbsOrigin() - delta, GetAbsOrigin() + delta,
		...
		for (int i = 0; i < count; i++)
		{
				...
				pSightEnt->GetHealth() > 0)
			{
				...
				if (IRelationship(pSightEnt) != R_NO && FInViewCone(pSightEnt) && !pSightEnt->GetFlags().
					Any(FL_NOTARGET) && FVisible(pSightEnt))
				{
...
CSound* CBaseMonster::PBestScent(void)
{
	...
	while (iThisScent != SOUNDLIST_EMPTY)
	{
		...
		if (pSound->FIsScent())
		{
			float flDist = (pSound->m_vecOrigin - GetAbsOrigin()).Length();
...
void CBaseMonster::CheckAttacks(CBaseEntity* pTarget, float flDist)
{
	...
	Vector2D vec2LOS = (pTarget->GetAbsOrigin() - GetAbsOrigin()).Make2D();
...
int CBaseMonster::CheckLocalMove(const Vector& vecStart, const Vector& vecEnd, const CBaseEntity* const pTarget,
                                 float* pflDist)
{
	...
	*/
	...
	if (iReturn == LOCALMOVE_VALID && !GetFlags().Any(FL_FLY | FL_SWIM) && (!pTarget || pTarget->GetFlags().
		Any(FL_ONGROUND)))
	{
...
bool CBaseMonster::BuildRoute(const Vector& vecGoal, int iMoveFlag, const CBaseEntity* const pTarget)
{
	...
	int iLocalMove = CheckLocalMove(GetAbsOrigin(), vecGoal, pTarget, &flDist);
	...
	if (iLocalMove == LOCALMOVE_VALID)
	{
		...
		GetAbsOrigin(), vecGoal, flDist, pTarget, &vecApex))
	{
...
bool CBaseMonster::FTriangulate(const Vector& vecStart, const Vector& vecEnd, float flDist,
                                const CBaseEntity* const pTargetEnt, Vector* pApex)
{
	Vector vecDir;
	Vector vecForward;
...
void CBaseMonster::StartMonster(void)
{
	...
	SetNextThink(RANDOM_FLOAT(0.1, 0.4)); // spread think times.
	...
	for (i = 0; i < WorldGraph.m_cNodes; i++)
	{
		...
		if (node.m_sHintType)
		{
			...
			if (FValidateHintType(node.m_sHintType))
			{
				...
				{
					UTIL_TraceLine(GetAbsOrigin() + GetViewOffset(), node.m_vecOrigin + GetViewOffset(),
...
bool CBaseMonster::FindLateralCover(const Vector& vecThreat, const Vector& vecViewOffset)
{
	...
	for (i = 0; i < COVER_CHECKS; i++)
	{
		...
		UTIL_TraceLine(vecThreat + vecViewOffset, vecLeftTest + GetViewOffset(), ignore_monsters, ignore_glass,
		               ENT(pev)/*pentIgnore*/, &tr);

		if (tr.flFraction != 1.0)
		{
			if (FValidateCover(vecLeftTest) && CheckLocalMove(GetAbsOrigin(), vecLeftTest, NULL, NULL) ==
				LOCALMOVE_VALID)
			{
		...
		// it's faster to check the SightEnt's visibility to the potential spot than to check the local move, so we do that first.
		UTIL_TraceLine(vecThreat + vecViewOffset, vecRightTest + GetViewOffset(), ignore_monsters, ignore_glass,
		               ENT(pev)/*pentIgnore*/, &tr);

		if (tr.flFraction != 1.0)
		{
			if (FValidateCover(vecRightTest) && CheckLocalMove(GetAbsOrigin(), vecRightTest, NULL, NULL) ==
				LOCALMOVE_VALID)
			{
...
void CBaseMonster::PlayScriptedSentence(const char* pszSentence, float duration, float volume, float attenuation,
                                        const bool bConcurrent, CBaseEntity* pListener)
{
