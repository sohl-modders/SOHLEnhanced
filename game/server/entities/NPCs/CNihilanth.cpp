			...
			if (RANDOM_LONG(0, 1) == 0)
			{
				...
				if (iNode != NO_NODE)
				{
					...
					UTIL_TraceHull(node.m_vecOrigin + Vector(0, 0, 32), node.m_vecOrigin + Vector(0, 0, 32),
					               dont_ignore_monsters, Hull::LARGE, NULL, &tr);
					if (tr.fStartSolid == 0)
			...
			else
			{
				...
				if (iNode != NO_NODE)
				{
					...
					UTIL_TraceHull(node.m_vecOrigin + Vector(0, 0, 36), node.m_vecOrigin + Vector(0, 0, 36),
					               dont_ignore_monsters, Hull::HUMAN, NULL, &tr);
					if (tr.fStartSolid == 0)
...
void CNihilanth::NextActivity()
{
	...
	if ((GetHealth() < gSkillData.GetNihilanthHealth() / 2 || m_iActiveSpheres < N_SPHERES / 2) && m_hRecharger == NULL
		&& m_iLevel <= 9)
	{
		char szName[64];
...
void CNihilanth::TargetSphere(USE_TYPE useType, float value)
{
	...
	pSphere->SetAbsVelocity(
		m_vecDesired * RANDOM_FLOAT(50, 100) + Vector(RANDOM_FLOAT(-50, 50), RANDOM_FLOAT(-50, 50),
		                                              RANDOM_FLOAT(-50, 50)));
}
	...
	case 3: // prayer
		if (m_hEnemy != NULL)
		{
			...
			if (pTrigger != NULL || pTouch != NULL)
			{
				...
				Vector vecSrc, vecAngles;
				GetAttachment(2, vecSrc, vecAngles);
				CNihilanthHVR* pEntity = (CNihilanthHVR*)Create("nihilanth_energy_ball", vecSrc, GetAbsAngles(),
				                                                edict());
				pEntity->SetAbsVelocity(GetAbsOrigin() - vecSrc);
...
void CNihilanth::TraceAttack(const CTakeDamageInfo& info, Vector vecDir, TraceResult& tr)
{
	...
	if (m_irritation != 3)
	{
		...
		UTIL_BloodStream(tr.vecEndPos, vecBlood, BloodColor(),
		                 info.GetDamage() + (100 - 100 * (GetHealth() / gSkillData.GetNihilanthHealth())));
	}
